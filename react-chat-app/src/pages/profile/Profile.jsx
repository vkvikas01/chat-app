...
function Profile() {
  ...
  const saveChanges = async () => {
    if (validateProfile()) {
      try {
        const response = await apiClient.post(UPDATE_PROFILE_ROUTE, { firstName, lastName, color: color }, { withCredentials: true });
      ...
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        ...
        <div className="grid grid-cols-2">
          ...
          >
            ...
            {hovered && (<div className='absolute insert-0 flex items-center justify-center bg-black/50 ring-fuchsia-50 cursor-pointer ' onClick={image ? handleDeleteImage : handleFileInputClick}>
              {
                image ? <FaTrash className='text-white text-3xl cursor-pointer' /> : <FaPlus className='text-white text-3xl cursor-pointer' />
              }
            </div>)}
            <input type='file' ref={fileInputRef} className='hidden' onChange={(event) => handleImageChange(event)} name="profile-image" accept='.png, .jpg, .jpeg, .svg, .webp' />
          </div>
